...
@if( has_feature( 'contacts_limit' ) )
	@if (in_array('contacts.index',$permissions))
		<li>
			<a href="{{ route('contacts.index') }}"><i class="ti-id-badge""></i><span>{{ _lang('Contacts') }}</span></a>
		</li>
	@endif
	@if (in_array('contact_groups.index',$permissions))
		<li>
			<a href="{{ url('contact_groups') }}"><i class="ti-id-badge"></i><span>{{ _lang('Contact Group') }}</span></a>
		</li>
	@endif
@endif
